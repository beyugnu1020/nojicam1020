  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-gradient-to-b from-green-100 to-white">
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-white border-t flex justify-around px-2 py-2 shadow-md">
        {["home", "register", "friends", "mypage", "admin"].map((tabKey, idx) => (
          <button
            key={tabKey}
            onClick={() => setTab(tabKey)}
            className={\`flex flex-col items-center text-sm \${tab === tabKey ? "text-green-600 font-bold" : "text-gray-500"}\`}
          >
            {["🏕️", "➕", "👥", "🙋", "⚙️"][idx]}
            <span>{["홈", "등록", "친구", "내정보", "관리자"][idx]}</span>
          </button>
        ))}
      </nav>
    </div>
